...
class Solution:
    def findMissingRanges(self, nums: List[int], lower: int, upper: int) -> List[List[int]]:
        ...
        for num in nums:
            # Check if there is a gap between start and the current number
            if num > start:
                end = num - 1
                self.add_range(missing_ranges, start, end)
            
            # Move the start to the next number
            start = num + 1
        
        # Check for missing range after the last element in the list
        if start <= upper:
            self.add_range(missing_ranges, start, upper)
        
        return missing_ranges
    
    def add_range(self, missing_ranges: List[List[int]], start: int, end: int) -> None:
